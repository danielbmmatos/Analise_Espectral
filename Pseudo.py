...
def Kanai_Tajimi(Ap,tipo,duraçao,dt,wg =0):
    """ 
    ...
    for i in range(tf):
        ...
        S =np.sqrt(2*Sg*df*2*np.pi)*np.cos(w*t[i]+P)
        ag[i] = sum(S)
    ...
    plt.xlim(0,20); plt.ylim(-max(ag)*1.1,max(ag)*1.1);plt.title('Aceleração sísmica')
    plt.grid(axis ='y')
#---------------------------------------------------------------------------------
    return t,ag,Sg,w
